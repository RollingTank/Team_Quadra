...
public class BetterOpenCV extends OpenCvPipeline {
    @Override
    public Mat processFrame(Mat input) {
        int c = 0;
        PropPositions propPosition = PropPositions.UNFOUND;
        int bottomAreaofBlock = 400, topAreaofBlock  = 600;
      ...
      //Mat temp = Imgcodecs.imread("redCube.jpg"); //Replace with "redCube.jpg" when needed
     // Imgproc.matchTemplate(gray, temp, gray, Imgproc.TM_CCOEFF_NORMED);
        ...
        Mat left = gray.submat(0, 150, 0, 150);
        Mat center = gray.submat(0, 150, 0, 150);

        Imgproc.findContours(gray, contourMapLeft, new Mat(), Imgproc.RETR_TREE, Imgproc.CHAIN_APPROX_SIMPLE);
        Imgproc.findContours(gray, contourMapCenter, new Mat(), Imgproc.RETR_TREE, Imgproc.CHAIN_APPROX_SIMPLE);
